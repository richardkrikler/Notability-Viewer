...
for (let i = 0; i < listItems.length; i++) {
  if (listItems[i].classList.contains('folder')) {
    listItems[i].addEventListener('click', () => showFolder(this.id))
...
function setPreview(listItem) {
  ...
  for (let i = 0; i < listItems.length; i++) {
    ...
    if (
      listItemClass != null &&
      listItemClass.item(0) == 'folder' &&
      listItems[i].textContent.trim().slice(3) == noteFolder
    ) {
  ...
  window.location.href =
    windowHref[0] + '#' + noteFolder + '/' + listItem.textContent
}
...
newTabFile.addEventListener('click', () =>
...
function sortListItems() {
  ...
  filesLi.sort((a, b) => {
    ...
    let aDate = a.innerText.substring(
      a.innerText.length - 14,
      a.innerText.length - 4
    )
    let bDate = b.innerText.substring(
      b.innerText.length - 14,
      b.innerText.length - 4
    )
...
copyTitle.addEventListener('click', () => copyStringToClipboard(currentTitle))

// copy the full file path of the currently open note into the clipboard
copyFilePath.addEventListener('click', () =>
...
function loadStorage() {
  ...
  if (recentNotes != null) {
    ...
    for (let i = 0; i < recentNotes.length; i++) {
      let frag = document
        .createRange()
        .createContextualFragment(recentNotes[i]).firstChild
  ...
  if (note != null) {
    ...
    for (let i = 0; i < listItems.length; i++) {
      ...
      if (listItemId.includes(note)) {
        ...
        showFolder(
          listItems[i].id.substring(0, listItems[i].id.lastIndexOf('\\'))
        )
      }
    }
  }
